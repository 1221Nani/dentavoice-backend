    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="col-span-1 card p-5 space-y-4 self-start">
          <h3 className="font-semibold text-ink-50">Report Builder</h3>
          <form onSubmit={handleGenerate} className="space-y-4">
              ...
              <div className="flex gap-1">
                {CHART_TYPES.map(t => (
                  <button key={t} type="button" onClick={() => setForm(f => ({...f, chart_type: t}))}
                    className={clsx('flex-1 py-1.5 rounded text-xs font-medium transition-all capitalize', form.chart_type === t ? 'bg-aurora-blue text-white' : 'bg-white/10 text-ink-500')}>
                    {t}
                  </button>
                ))}
              </div>
              ...
              <div className="flex gap-1">
                {GROUP_BY.map(g => (
                  <button key={g} type="button" onClick={() => setForm(f => ({...f, group_by: g}))}
                    className={clsx('flex-1 py-1.5 rounded text-xs font-medium transition-all capitalize', form.group_by === g ? 'bg-aurora-blue text-white' : 'bg-white/10 text-ink-500')}>
                    {g}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {METRICS.map(m => (
                  <button key={m.key} type="button" onClick={() => toggleMetric(m.key)}
                    className={clsx('px-2.5 py-1 rounded text-xs font-medium transition-all', form.metrics.includes(m.key) ? 'bg-aurora-blue text-white' : 'bg-white/10 text-ink-500 hover:bg-white/10')}>
                    {m.key.toUpperCase()}
                  </button>
                ))}
              </div>
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.narrate}
                ...
                className="rounded border-white/20 text-aurora-blue" />
              <span className="text-sm text-ink-300 flex items-center gap-1">
                <Brain size={13} className="text-aurora-indigo" />
                Add AI Narration
              </span>
            </label>
        ...
        <div className="col-span-3 space-y-5">
          {!report && !loading && (
            <div className="card p-12 text-center text-ink-500">
              ...
              <p className="font-medium text-ink-300">Configure and generate your report</p>
          ...
          {loading && (
            <div className="card p-12 text-center">
              ...
              <p className="text-ink-500">Building report...</p>
            </div>
          )}
          ...
          {report && (
            <>
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div>
                  <h2 className="text-xl font-bold text-ink-50">{report.title}</h2>
                  <p className="text-sm text-ink-500">{report.start_date} → {report.end_date} · {report.platform === 'all' ? 'All Platforms' : report.platform}</p>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  <button onClick={exportCSV}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-base-800 border border-white/10 text-ink-300 hover:bg-white/5 rounded-lg transition-colors">
                  ...
                  <button onClick={copyShareLink}
                    className={clsx('flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border rounded-lg transition-colors',
                      copied ? 'bg-green-500/10 border-green-500/25 text-green-400' : 'bg-base-800 border-white/10 text-ink-300 hover:bg-white/5')}>
                    <Link size={13} /> {copied ? 'Copied!' : 'Share Link'}
                  </button>
                  <p className="text-xs text-ink-700">Generated {report.generated_at}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={l} className="card p-4 text-center">
                    <p className="text-2xl font-bold text-ink-50">{v}</p>
              ...
              <div className="card p-5">
                <h3 className="font-semibold text-ink-50 mb-4">
                  {form.metrics.map(m => METRICS.find(x => x.key === m)?.label).join(' · ')} over time
                </h3>
                {report.chart_data.length === 0 ? (
                  <div className="h-64 flex items-center justify-center text-ink-500">No data for selected period</div>
                ) : (
              ...
              {report.narration && (
                <div className="card p-5 space-y-4">
                  <div className="flex items-center gap-2 pb-3 border-b border-white/10">
                    <Brain size={16} className="text-aurora-indigo" />
                    <h3 className="font-semibold text-ink-50">AI Report Analysis</h3>
                  </div>
                  {report.narration.headline && (
                    <p className="text-base font-medium text-ink-50">{report.narration.headline}</p>
                  )}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {[
                      { key: 'spend_narrative', label: 'Spend', icon: TrendingUp, color: 'text-aurora-blue' },
                      { key: 'revenue_narrative', label: 'Revenue & ROAS', icon: TrendingUp, color: 'text-green-400' },
                      { key: 'ctr_narrative', label: 'CTR', icon: TrendingUp, color: 'text-aurora-coral' },
                      { key: 'conversion_narrative', label: 'Conversions', icon: TrendingUp, color: 'text-aurora-indigo' },
                    ].map(({ key, label, icon: Icon, color }) => report.narration[key] && (
                      <div key={key} className="bg-white/5 rounded-xl p-3">
                        <div className={`text-xs font-semibold ${color} mb-1`}>{label}</div>
                        <p className="text-sm text-ink-300">{report.narration[key]}</p>
                      </div>
                    ))}
                  </div>
                  {report.narration.top_performer && (
                    <div className="flex items-start gap-2 p-3 bg-green-500/10 rounded-xl">
                      ...
                      <p className="text-sm text-green-400">{report.narration.top_performer}</p>
                    </div>
                  )}
                  {report.narration.concern && (
                    <div className="flex items-start gap-2 p-3 bg-yellow-500/10 rounded-xl">
                      <AlertTriangle size={14} className="text-yellow-400 mt-0.5 shrink-0" />
                      <p className="text-sm text-yellow-400">{report.narration.concern}</p>
                    </div>
                  )}
                  {report.narration.recommendation && (
                    <div className="flex items-start gap-2 p-3 bg-aurora-indigo/10 rounded-xl border border-aurora-indigo/20">
                      <Lightbulb size={14} className="text-aurora-indigo mt-0.5 shrink-0" />
                      <p className="text-sm text-aurora-violet font-medium">{report.narration.recommendation}</p>
                    </div>
                  )}
                </div>
              )}
                      ...
                      <thead>
                        <tr className="border-b border-white/5 bg-white/5">
                          {['Campaign', 'Spend', 'Revenue', 'ROAS', 'Clicks', 'Impressions', 'CTR', 'Conv.'].map(h => (
                            <th key={h} className="px-4 py-3 text-left text-xs font-medium text-ink-500 uppercase tracking-wide whitespace-nowrap">{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {report.campaign_breakdown.map((row) => (
                          <tr key={row.campaign_id} className="border-b border-white/5 hover:bg-white/5">
                            <td className="px-4 py-3 font-medium text-ink-50 whitespace-nowrap">{row.campaign_name}</td>
                            <td className="px-4 py-3 tabular-nums">{fmt.currency(row.spend)}</td>
                            <td className="px-4 py-3 tabular-nums text-green-400">{fmt.currency(row.revenue)}</td>
