    ...
    <div className="space-y-6">
      {loadError && (
        <div className="flex items-center gap-2 px-4 py-3 bg-red-500/10 border border-red-500/25 rounded-xl text-sm text-red-400">
          ...
          {loadError}
          <button onClick={load} className="ml-auto text-xs font-medium text-red-400 hover:underline">Retry</button>
        </div>
      )}
      {/* Data freshness warning */}
      {freshness && freshness.stale && (
        <div className="flex items-start gap-3 px-4 py-3 bg-yellow-500/10 border border-yellow-500/25 rounded-xl text-sm text-yellow-400">
          ...
          <a href="/performance" className="flex items-center gap-1 text-xs font-medium text-yellow-400 border border-yellow-500/30 px-2.5 py-1.5 rounded-lg hover:bg-yellow-500/15 whitespace-nowrap">
            <CloudDownload size={12} /> Sync Data
          </a>
        </div>
      )}
      {freshness && !freshness.stale && freshness.has_data && (
        <div className="flex items-center gap-2 px-4 py-2 bg-green-500/10 border border-green-500/20 rounded-xl text-xs text-green-400">
      ...
      <div className="card p-4">
        <div className="flex flex-wrap items-center gap-3">
          <span className="text-sm font-medium text-ink-300">Analyze campaigns:</span>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 flex-wrap items-center">
          ...
          {(typeFilter || impactFilter) && (
            <button onClick={() => { setTypeFilter(''); setImpactFilter('') }} className="text-xs text-red-400 hover:underline">Clear</button>
          )}
      ...
      {generating && (
        <div className="card p-6 flex items-center gap-4">
          <div className="w-10 h-10 border-4 border-aurora-blue/25 border-t-blue-600 rounded-full animate-spin flex-shrink-0" />
          <div>
            <p className="font-medium text-ink-50">Analyzing campaign performance...</p>
            <p className="text-sm text-ink-500">Reviewing your metrics and generating actionable recommendations</p>
          </div>
        </div>
      )}

      {generateError && !generating && (
        <div className="card p-4 bg-yellow-500/10 border border-yellow-500/20 flex items-start gap-2 text-sm text-yellow-400">
          <XCircle size={15} className="shrink-0 mt-0.5 text-yellow-400" />
          <span>{generateError}</span>
        </div>
      )}
      {generateInfo && !generating && !generateError && (
        <div className="card p-4 bg-aurora-blue/10 border border-aurora-blue/20 flex items-start gap-2 text-sm text-aurora-blue">
          <CheckCircle size={15} className="shrink-0 mt-0.5 text-aurora-blue" />
      ...
      {recs.length === 0 && tab === 'pending' && !loading && !generating && (
        <div className="card p-10 text-center">
          <div className="w-14 h-14 bg-aurora-blue/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Zap size={24} className="text-aurora-blue" />
          </div>
          <h3 className="font-semibold text-ink-50 mb-2">No pending recommendations</h3>
          <p className="text-ink-500 text-sm mb-4">Click "Generate AI Recommendations" to analyze your campaigns and get actionable insights.</p>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <h4 className="font-semibold text-ink-50">{rec.title}</h4>
                    ...
                    <span className="text-xs text-ink-500 capitalize px-2 py-0.5 bg-white/5 rounded-full">{rec.type}</span>
                    {rec.status !== 'pending' && (
                      <span className={clsx('text-xs font-medium px-2 py-0.5 rounded-full', rec.status === 'applied' ? 'bg-green-500/10 text-green-400' : 'bg-white/5 text-ink-500')}>
                        {rec.status}
                      </span>
                    )}
                  </div>
                  <p className="text-sm text-ink-300 leading-relaxed">{rec.description}</p>
                  {rec.action?.estimated_improvement && (
                    <p className="text-xs text-aurora-blue mt-2 font-medium">Estimated: {rec.action.estimated_improvement}</p>
                  )}
                  <p className="text-xs text-ink-700 mt-2">{new Date(rec.created_at).toLocaleString()}</p>
                </div>
                {!isHistory && (
                  <div className="flex gap-2 flex-shrink-0">
                    <button onClick={() => handleApply(rec.id)}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-green-500 text-white text-xs font-medium rounded-lg hover:bg-green-500 transition-colors">
                      <CheckCircle size={13} /> Apply
                    </button>
                    <button onClick={() => handleDismiss(rec.id)}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-white/10 text-ink-300 text-xs font-medium rounded-lg hover:bg-white/10 transition-colors">
      ...
      {tab === 'pending' && recs.length > 0 && (
        <div className="card p-5 bg-aurora-blue/10 border-aurora-blue/20">
          <h4 className="font-medium text-ink-50 mb-1">Auto-Apply Rules</h4>
          <p className="text-sm text-aurora-blue">Coming soon: Set rules to automatically apply budget changes, pause underperforming campaigns, and scale top performers without manual review.</p>
        </div>
      )}
    </div>
  )
